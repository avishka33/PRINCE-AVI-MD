...
async function RentBot(conn, from, m, store) {
	async function startRentBot() {
		try {
			const { state, saveCreds } = await useMultiFileAuthState(`./database/RentBot/${from}`);
			...
			client[from].ev.on('connection.update', async (update) => {
				...
				if (connection === 'connecting' && !client[from].authState.creds.registered && !client[from].pairingStarted) {
					setTimeout(async () => {
						...
						exec('rm -rf ./database/RentBot/' + from + '/*');
				...
				if (connection === 'close') {
					...
					if ([DisconnectReason.connectionLost, DisconnectReason.connectionClosed, DisconnectReason.restartRequired, DisconnectReason.timedOut, DisconnectReason.badSession, DisconnectReason.connectionReplaced].includes(reason)) {
						RentBot(conn, from, m, store)
					} else if (reason === DisconnectReason.loggedOut) {
						m.reply('Scan again and Run...');
						StopRentBot(conn, from, m)
			...
			console.log('Error di RentBot : ', e)
		}
	}
	return startRentBot()
}

async function StopRentBot(conn, from, m) {
	if (!Object.keys(client).includes(from)) {
		return conn.sendMessage(m.chat, { text: 'Anda Tidak Sedang RentBot!' }, { quoted: m })
	}
	try {
		client[from].end('Stop')
		client[from].ev.removeAllListeners()
	} catch (e) {
		console.log('Errornya di stopRentBot : ', e)
	}
	delete client[from]
	exec(`rm -rf ./database/RentBot/${from}`)
...
async function ListRentBot(conn, m) {
	let teks = 'List Jadi Bot :\n\n'
	for (let RentBot of Object.values(client)) {
		teks += (RentBot.user?.id ? `- @${conn.decodeJid(RentBot.user.id).split('@')[0]}\n` : '')
	}
	return m.reply(teks)
}

module.exports = { RentBot, StopRentBot, ListRentBot }