...
async function GroupUpdate(Avishka, m, store) {
	...
	if (global.db?.groups?.[m.chat]?.setinfo && Avishka.public) {
		...
		if (messages[m.messageStubType]) {
			await Avishka.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(m.messageStubParameters[0]?.includes('@') ? [`${m.messageStubParameters[0]}`] : [])]}, { ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else {
			console.log({
...
async function GroupParticipantsUpdate(Avishka, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await Avishka.profilePictureUrl(n, 'image');
				} catch {
...
async function MessagesUpsert(Avishka, message, store, groupCache) {
	try {
		...
		const m = await Serialize(Avishka, msg, store, groupCache)
		require('../Avishka')(Avishka, m, msg, store, groupCache);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await Avishka.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await Avishka.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await Avishka.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(Avishka, store) {
	...
	Avishka.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		...
		Avishka.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	Avishka.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return Avishka.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	...
	Avishka.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return Avishka.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return Avishka.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return Avishka.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return Avishka.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return Avishka.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return Avishka.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	Avishka.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
	...
	Avishka.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await Avishka.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	Avishka.sendText = async (jid, text, quoted, options = {}) => Avishka.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	...
	Avishka.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return Avishka.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	...
	Avishka.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await Avishka.downloadMediaMessage(message);
	...
	Avishka.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: Avishka.user.id, quoted: m.quoted });
		apb.key = m.key
	...
	Avishka.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await Avishka.getFile(path, true);
		...
		let anu = await Avishka.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
...
async function Serialize(Avishka, msg, store, groupCache) {
	...
	if (m.key) {
		...
		m.sender = Avishka.decodeJid(m.fromMe && Avishka.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			...
			if (!metadata) {
				metadata = await Avishka.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = Avishka.decodeJid(m.msg.contextInfo.participant)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, Avishka)
				return await Serialize(Avishka, q, store, groupCache)
			}
			m.quoted.key = {
	...
	m.copy = () => Serialize(Avishka, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return Avishka.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return Avishka.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return Avishka.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
					}
				} else {
					return Avishka.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
				}
